/* Adapted from https://github.com/nimiq/qr-scanner */
...
export default class QrScanner {
	...
	readonly $overlayScanRegion?: SVGSVGElement
	readonly $overlayAnimation?: Animation
	$overlayColor: string
	...
	private _qrEnginePromise: Promise<Worker>
	...
	constructor(
		...
		onDecode: ((result: ScanResult) => void) | ((result: string) => void),
		options: {
		...
		canvasSizeOrCalculateScanRegion?:
			| number
			| ((video: HTMLVideoElement) => ScanRegion),
		preferredCamera?: FacingMode | DeviceId,
	) {
		this.$video = video
		...
		this.$overlayColor = options.overlayColor || '#4a90e2'
				...
				this.$overlay.innerHTML = `
					<svg
						...
						style="position:absolute;width:100%;height:100%;left:0;top:0;
							fill:none;stroke:${this.$overlayColor};stroke-width:4;stroke-linecap:round;stroke-linejoin:round">
								<path d="M31 2H10a8 8 0 0 0-8 8v21M207 2h21a8 8 0 0 1 8 8v21m0 176v21a8 8 0 0 1-8 8h-21m-176
									0H10a8 8 0 0 1-8-8v-21"/></svg>`
				this.$overlayScanRegion = this.$overlay
					.firstElementChild as SVGSVGElement
				try {
					this.$overlayAnimation = this.$overlayScanRegion!.animate(
					...
					this.$overlayAnimation.addEventListener('finish', () => {
			...
			if (options.highlightCodeOutline) {
				...
				this.$overlay.insertAdjacentHTML(
					...
					`<svg class="code-outline-highlight" preserveAspectRatio="none" style="display:none;width:100%;
						height:100%;fill:none;stroke:${this.$overlayColor};stroke-width:5;stroke-dasharray:25;
		...
		requestAnimationFrame(() => {
			...
			if (shouldHideVideo) {
				...
				// @ts-ignore
				this.$codeOutlineHighlight = undefined
			}
	...
	static async scanImage(
		...
		options?: {
			...
			qrEngine?: Worker | Promise<Worker> | null
			canvas?: HTMLCanvasElement | null
			disallowCanvasResizing?: boolean
			alsoTryWithoutScanRegion?: boolean
		} | null,
	): Promise<ScanResult> {
		let scanRegion = options?.scanRegion
		...
		try {
			...
			const [qrEngineWorker, image] = await Promise.all([
	...
	static async createQrEngine(): Promise<Worker> {
	...
	private _updateOverlay(): void {
		requestAnimationFrame(() => {
			...
			overlayStyle.height = `${
				(regionHeight / videoHeight) * videoScaledHeight
			}px`
			overlayStyle.top = `${
	...
	updateOverlayColor(color: string): void {
		this.$overlayColor = color
		if (this.$overlayScanRegion) {
			this.$overlayScanRegion.style.stroke = color
		}
		if (this.$codeOutlineHighlight) {
			this.$codeOutlineHighlight.style.stroke = color
		}
	}
	...
	private _scanFrame(): void {
		...
		requestFrame(async () => {
			...
			if (!((await this._qrEnginePromise) instanceof Worker)) {
				this._qrEnginePromise = QrScanner.createQrEngine()
			}
	...
	private static async _postWorkerMessage(
		qrEngineOrQrEnginePromise: Worker | Promise<Worker>,
		type: string,
		data?: any,
